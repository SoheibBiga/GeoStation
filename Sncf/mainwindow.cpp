    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->setupUi(this);
    close();
    manager_liste_gare = new QNetworkAccessManager(this);
    manager_prochain_depart = new QNetworkAccessManager(this);
    connect(manager_liste_gare, SIGNAL(finished(QNetworkReply*)), this, SLOT(result_liste_gare(QNetworkReply*)));
...
void MainWindow::result_liste_gare(QNetworkReply* reply)
{
...
void MainWindow::result_prochain_depart(QNetworkReply* reply)
{
    //if (reply->error() != QNetworkReply::NoError)
       // return;  // ...only in a blog post
