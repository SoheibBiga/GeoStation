...
void PollutionMozaicWidget::receive_info2(QMap<QString, QVariant> map_ameliore)
{
    ...
    if(n<=50){
        ...
        ui->textEdit->setStyleSheet("background:#f00;color:white;font-size:20px");
        ui->textEdit->setAlignment(Qt::AlignCenter);
        //ui->textEdit->setFont(m_font);
        ui->textEdit->setText(QString(" Indice de qualité de l'air :\t\n%1\nMauvais!!").arg(QString::number(n)));
     }else if (n<=300) {
        ui->textEdit->setStyleSheet("background:#99004c;color:white;font-size:20px");
        ui->textEdit->setAlignment(Qt::AlignCenter);
        //ui->textEdit->setFont(m_font);
        ui->textEdit->setText(QString(" Indice de qualité de l'air :\t\n%1\nTrès mauvais!!").arg(QString::number(n)));
     }else{
        ui->textEdit->setStyleSheet("background:#7e0023;color:white;font-size:20px");
        ui->textEdit->setAlignment(Qt::AlignCenter);
        //ui->textEdit->setFont(m_font);
        ui->textEdit->setText(QString(" Indice de qualité de l'air :\t\n%1\nDangereux!!").arg(QString::number(n)));
     }
    //ui->label2->setFont(t_font);
    ui->label2->setStyleSheet("background:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgba(0, 228, 0, 1), stop:0.166 rgba(255, 255, 0, 1), stop:0.333 rgba(255, 126, 0, 1), stop:0.5 rgba(255, 0, 0, 1), stop:0.666 rgba(153, 0, 76, 1), stop:0.833 rgba(126, 0, 35, 1));color:#c7bead;font-size:30px;font-weight: bold");
